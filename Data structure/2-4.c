...
//평균을 구하는 함수
double get_mean(student *students, int n)  //배열 students포인터로 입력받고, 학생수를 정수 n으로 입력받음
{
	int i = 0;
...
//표준편차를 구하는 함수
double get_deviation(student *students, int n) //배열 students포인터로 입력받고, 학생수를 정수 n으로 입력받음
{
	int i; //for문을 위한 i변수, 총합을 위한 sum 변수, 평균을 위한 mean 변수, 
	double sum1 = 0, sum2 = 0, mean, sum = 0; // math함수 pow와 sqrt는 double로 처리하기 때문에 double로 선언
	for (i = 0; i < n; i++)
	{
		sum += (*(students + i)).score; //students배열의 score점수의 값을 sum에 더함
	}
